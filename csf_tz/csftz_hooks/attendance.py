...
def process_overtime(doc, method):
    if not frappe.db.get_single_value("CSF TZ Settings", "enable_overtime"):
        return
    
    if (
        doc.status != "Present" or
        not doc.overtime_applicable or
        not doc.in_time or
        not doc.out_time
    ):
    ...
    if shift_type.enable_entry_grace_period == 1 and shift_type.late_entry_grace_period != None:
        late_entry_grace_period = shift_type.late_entry_grace_period

    if shift_type.enable_exit_grace_period == 1 and shift_type.early_exit_grace_period != None:
    ...
    shift_start_time, excess_in_time = calculate_shift_start_time(checkin_time, start_time, late_entry_grace_period)
    shift_end_time, excess_out_time = calculate_shift_end_time(checkout_time, end_time, early_exit_grace_period)
    ...
    set_eligible_and_excess_overtime(doc, excess_in_time, excess_out_time, eligible_working_hours, threshold, is_holiday)
...
def calculate_shift_start_time(checkin_time, start_time, late_entry_grace_period):
    ...
    if checkin_time < get_time(str(start_time)):
        shift_start_time = get_time(str(start_time))
        ...
        if late_entry_grace_period != None:
            ...
            if checkin_time <= late_entry_datetime:
                shift_start_time = get_time(str(start_time))
            else:
...
def calculate_shift_end_time(checkout_time, end_time, early_exit_grace_period):
    ...
    if checkout_time > get_time(str(end_time)):
        shift_end_time = get_time(str(end_time))
        ...
        if early_exit_grace_period != None:
            ...
            if checkout_time >= early_exit_datetime:
                shift_end_time = get_time(str(end_time))
            else:
...
def set_eligible_and_excess_overtime(doc, excess_in_time, excess_out_time, eligible_working_hours, threshold, is_holiday):
    eligible_overtime_normal = None
    excess_overtime_normal = None
    eligible_overtime_holiday = None
    excess_overtime_holiday = None

    excess_in_ = datetime.strptime(str(excess_in_time), "%H:%M:%S")
    excess_out_ = datetime.strptime(str(excess_out_time), "%H:%M:%S")

    if is_holiday == False:
        eligible_overtime_normal = time_diff(str(eligible_working_hours), str(threshold))
        excess_overtime_normal = excess_in_ + excess_out_
    else:
        eligible_overtime_holiday = eligible_working_hours
        excess_overtime_holiday = excess_in_ + excess_out_
    
    doc.eligible_working_hours = eligible_working_hours or "00:00:00"

    if doc.on_approval_overtime == 1:
        if eligible_overtime_normal:
            overtime_normal = datetime.strptime(str(eligible_overtime_normal), "%H:%M:%S")
        else:
        ...
        if eligible_overtime_holiday:
            overtime_holiday = datetime.strptime(str(eligible_overtime_holiday), "%H:%M:%S")
        else:
        ...
        eligible_overtime_normal___ = "00:00:00" if "day" in str(eligible_overtime_normal) else eligible_overtime_normal
        eligible_overtime_holiday___ = "00:00:00" if "day" in str(eligible_overtime_holiday) else eligible_overtime_holiday
